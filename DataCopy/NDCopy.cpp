...
#include <functional>
#include <stack>
//NdCopyRecurDepthFirstSeqPaddingAlgm(): helper function
...
static void NdCopyRecurDepthFirstSeqPaddingAlgm(size_t curDim,
                    char*& inOvlpBase,
                    char*& outOvlpBase,
                    Dims& inOvlpGapSize,
                    Dims& outOvlpGapSize,
                    ...
                    size_t& blockSize)
{
  ...
  //case: curDim<minCountDim
    ...
    for (size_t i=0; i<ovlpCount[curDim];i++){
      NdCopyRecurDepthFirstSeqPaddingAlgm(curDim+1,
              inOvlpBase,
              outOvlpBase,
              inOvlpGapSize,
              outOvlpGapSize,
...
static void NdCopyIterDepthFirstSeqPaddingAlgm (
                              char*& inOvlpBase,
                              char*& outOvlpBase,
                              Dims& inOvlpGapSize,
                              Dims& outOvlpGapSize,
...
static void NdCopyRecurDepthFirstSeqPaddingRevEndianAlgm(size_t curDim,
                          char*& inOvlpBase,
                          char*& outOvlpBase,
                          Dims& inOvlpGapSize,
                          Dims& outOvlpGapSize,
                          ...
                          size_t numElmsPerBlock)
{
  ...
  //case: curDim<minCountDim
    ...
    for (size_t i=0; i<ovlpCount[curDim];i++)
      NdCopyRecurDepthFirstSeqPaddingRevEndianAlgm(curDim+1,inOvlpBase,outOvlpBase,inOvlpGapSize,
...
static void NdCopyIterDepthFirstSeqPaddingRevEndianAlgm (
                                    char*& inOvlpBase,
                                    char*& outOvlpBase,
                                    Dims& inOvlpGapSize,
                                    Dims& outOvlpGapSize,
...
//minimized to average O(1), which is independent of the number of dimensions.
static void NdCopyRecurDepthFirstDynamicAlgm(size_t curDim,
                           char* inBase,
                           char* outBase,
                           Dims& inRltvOvlpSPos,
                           ...
                           size_t elmSize)
{
  if (curDim==inStride.size()){
    ...
    for (size_t i=0; i<ovlpCount[curDim];i++)
      NdCopyRecurDepthFirstDynamicAlgm(curDim+1,
...
static void NdCopyIterDepthFirstDynamicAlgm(
                                    char* inBase,
                                    char* outBase,
                                    Dims& inRltvOvlpSPos,
                                    Dims& outRltvOvlpSPos,
...
//minimized to average O(1), which is independent of the number of dimensions.
static void NdCopyRecurDepthFirstDynamicRevEndianAlgm(size_t curDim,
                                 char* inBase,
                                 char* outBase,
                                 Dims& inRltvOvlpSPos,
                                 Dims& outRltvOvlpSPos,
                                 ...
                                 size_t elmSize)
{
  if (curDim==inStride.size()){
    ...
    for (size_t i=0; i<ovlpCount[curDim];i++)
      NdCopyRecurDepthFirstDynamicRevEndianAlgm(curDim+1,
                           inBase+(inRltvOvlpSPos[curDim]+i)*inStride[curDim],
                           outBase+(outRltvOvlpSPos[curDim]+i)*outStride[curDim],
                           inRltvOvlpSPos,
                           outRltvOvlpSPos,
...
//performance is 50% slower than the recursive version
static void NdCopyIterDepthFirstDynamicRevEndianAlgm(
                                          char* inBase,
                                          char* outBase,
                                          Dims& inRltvOvlpSPos,
                                          Dims& outRltvOvlpSPos,
...
int NdCopy(const Buffer &in, const Dims &inStart, const Dims &inCount,
           bool inIsRowMaj, bool inIsBigEndian,
           Buffer &out, const Dims &outStart,
           ...
           bool safeMode)
{
  ...
  if (inIsRowMaj==true && outIsRowMaj==true)
  {
    ...
    //same endianess mode: most optimized, contiguous data copying algorithm used.
    if (inIsBigEndian==outIsBigEndian)
    {
      //most efficient algm
      //warning: number of function stacks used is number of dimensions of data.
      if (!safeMode)
        NdCopyRecurDepthFirstSeqPaddingAlgm(0, inOvlpBase, outOvlpBase, inOvlpGapSize,
                                            outOvlpGapSize, ovlpCount, minContDim,
                                            blockSize);
      else //safeMode
        //      //alternative iterative version, 10% slower then recursive
        //      //use it when very high demension is used.
        NdCopyIterDepthFirstSeqPaddingAlgm(inOvlpBase, outOvlpBase,
    ...
    {
      if (!safeMode)
        NdCopyRecurDepthFirstSeqPaddingRevEndianAlgm(0, inOvlpBase, outOvlpBase, inOvlpGapSize,
                                                     outOvlpGapSize, ovlpCount, minContDim,
                                                     blockSize, sizeof(T),
                                                     blockSize / sizeof(T));
      else
        NdCopyIterDepthFirstSeqPaddingRevEndianAlgm(inOvlpBase, outOvlpBase, inOvlpGapSize,
                                                    outOvlpGapSize, ovlpCount, minContDim,
                                                    blockSize, sizeof(T),
                                                    blockSize / sizeof(T));
    }
  ...
  //1. mem ptr arithmetics: O(1) overhead per block, dynamic/non-sequential padding
  else {
    ...
    {
      if (!safeMode)
        NdCopyRecurDepthFirstDynamicAlgm(0, inOvlpBase, outOvlpBase,
                                         inRltvOvlpStartPos, outRltvOvlpStartPos,
                                         inStride, outStride, ovlpCount, sizeof(T));
      else
        NdCopyIterDepthFirstDynamicAlgm(inOvlpBase, outOvlpBase,
                                        inRltvOvlpStartPos, outRltvOvlpStartPos,
                                        inStride, outStride, ovlpCount, sizeof(T));
    }
    //different Endian"
    ...
    {
      if (!safeMode)
        NdCopyRecurDepthFirstDynamicRevEndianAlgm(
                                                  0, inOvlpBase, outOvlpBase, inRltvOvlpStartPos,
                                                  outRltvOvlpStartPos, inStride, outStride, ovlpCount, sizeof(T));
      else
        NdCopyIterDepthFirstDynamicRevEndianAlgm(
                                                 inOvlpBase, outOvlpBase, inRltvOvlpStartPos,
                                                 outRltvOvlpStartPos, inStride, outStride, ovlpCount, sizeof(T));
    }
  }
  return 0;
