...
static void NdCopyRecurDFSeqPadding(size_t curDim,
                    char*& inOvlpBase,
                    char*& outOvlpBase,
                    Dims& inOvlpGapSize,
                    Dims& outOvlpGapSize,
                    ...
                    size_t& blockSize)
{
  ...
  //case: curDim<minCountDim
    ...
    for (size_t i=0; i<ovlpCount[curDim];i++){
      NdCopyRecurDFSeqPadding(curDim+1,
              inOvlpBase,
              outOvlpBase,
              inOvlpGapSize,
              outOvlpGapSize,
...
static void NdCopyIterDFSeqPadding (
                              char*& inOvlpBase,
                              char*& outOvlpBase,
                              Dims& inOvlpGapSize,
                              Dims& outOvlpGapSize,
...
static void NdCopyRecurDFSeqPaddingRevEndian(size_t curDim,
                          char*& inOvlpBase,
                          char*& outOvlpBase,
                          Dims& inOvlpGapSize,
                          Dims& outOvlpGapSize,
...
//minimized to average O(1), which is independent of the number of dimensions.
static void NdCopyRecurDFNonSeqDynamic(size_t curDim,
                           char* inBase,
                           char* outBase,
                           Dims& inRltvOvlpSPos,
                           ...
                           size_t elmSize)
{
  if (curDim==inStride.size()){
    ...
    for (size_t i=0; i<ovlpCount[curDim];i++)
      NdCopyRecurDFNonSeqDynamic(curDim+1,
...
static void NdCopyIterDFDynamic(
                                    char* inBase,
                                    char* outBase,
                                    Dims& inRltvOvlpSPos,
                                    Dims& outRltvOvlpSPos,
...
//minimized to average O(1), which is independent of the number of dimensions.
static void NdCopyRecurDFNonSeqDynamicRevEndian(size_t curDim,
                                 char* inBase,
                                 char* outBase,
                                 Dims& inRltvOvlpSPos,
                                 Dims& outRltvOvlpSPos,
                                 ...
                                 size_t elmSize)
{
  if (curDim==inStride.size()){
    ...
    for (size_t i=0; i<ovlpCount[curDim];i++)
      NdCopyRecurDFNonSeqDynamicRevEndian(curDim+1,
                           inBase+(inRltvOvlpSPos[curDim]+i)*inStride[curDim],
                           outBase+(outRltvOvlpSPos[curDim]+i)*outStride[curDim],
                           inRltvOvlpSPos,
                           outRltvOvlpSPos,
...
//performance is 50% slower than the recursive version
static void NdCopyIterDFDynamicRevEndian(
                                          char* inBase,
                                          char* outBase,
                                          Dims& inRltvOvlpSPos,
                                          Dims& outRltvOvlpSPos,
           ...
           bool safeMode)
{
  ...
  if (inIsRowMaj==true && outIsRowMaj==true)
  {
    ...
    if (inIsBigEndian==outIsBigEndian)
    {
      ...
      if (!safeMode)
        NdCopyRecurDFSeqPadding(0, inOvlpBase, outOvlpBase, inOvlpGapSize,
        ...
        NdCopyIterDFSeqPadding(inOvlpBase, outOvlpBase,
  ...
  //1. mem ptr arithmetics: O(1) overhead per block, dynamic/non-sequential padding
    ...
    if (inIsBigEndian==outIsBigEndian)
    {
      if (!safeMode)
        ...
        NdCopyIterDFDynamic(inOvlpBase, outOvlpBase,
                                        inRltvOvlpStartPos, outRltvOvlpStartPos,
                                        inStride, outStride, ovlpCount, sizeof(T));
    }
    //different Endian"
...
template <class T>
int NdCopyT(char *in, const Dims &inStart,
           ...
           bool outIsRowMaj, bool outIsBigEndian, bool safeMode)
{
  ...
  if (inIsRowMaj == true && outIsRowMaj == true)
  {
    ...
    if (inIsBigEndian == outIsBigEndian)
    {
      ...
      // of data.
      if (!safeMode){
        std::cout<<"from ndcopyT(), quick mode recursive"<<std::endl;
